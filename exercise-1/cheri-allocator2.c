...
alloc_allocate(void)
{
	...
	// to implement spatial safety, bounds & permissions are set on returned pointers
  #else		
 ...
 */
static void
alloc_free(void *ptr) // pointer to a_bytes (to be freed allocated mem)
{
	struct alloc_storage *alloc; // pointer to metadata (container struct)

#ifdef __CHERI_PURE_CAPABILITY__
   /*
    * Generate a new pointer to the allocation with bounds reset
	* that is derived from the one passed by the consumer.
    */		
	ptr = cheri_address_set(alloc_array, cheri_address_get(ptr));
#endif	
	/* Convert pointer to allocated memory into pointer to metadata. */
	alloc = __containerof(ptr, struct alloc_storage, a_bytes); // pointer type conversion issue
	
 	alloc->a_next = alloc_nextfree; // SIGPROT here. capability monotonity issue.
